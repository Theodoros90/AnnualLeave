...
**Full-replace update DTOs:** `AdminUpdateUserDto` is a replace, not a patch — `UpdateAdminUser` assigns every field it carries unconditionally, so a `null` in the request genuinely clears the stored value. Anything added to it has to follow that: mixing in a "a null leaves the stored answer alone" rule for one field gives a field that looks editable but quietly refuses to be cleared. A field that is genuinely optional on such a DTO therefore needs a way for the UI to *send* null. `User.Gender` used to be one — the dialog offered an explicit "Not specified" radio for exactly this reason — but it is now **required** on both admin validators (`PersonFieldRules.GenderRequiredMessage`), so the radio is gone; see the `User` row below for why.
...
- **A stored `null` gender passes.** It means "nobody entered it", which is every
  ...
  `null` can no longer be *chosen*: the admin dialogs offer Male or Female only
  and both admin validators refuse a save without one, so the nulls this tolerates
  are legacy rows on their way out, not a standing "offer me everything" answer.
