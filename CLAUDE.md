...
- **A stored `null` gender passes.** It means "nobody entered it", which is every
  ...
  "offer me everything" answer. The one standing `null` is an **Admin's** — the
  field is hidden and refused for that role — so an Admin filing their own leave
...
- **A `null` start date passes**, on both sides. It means nobody entered it — an
  Admin never has one, since the validators refuse it for the role, and an Employee
